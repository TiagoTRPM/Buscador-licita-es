...
def _parse_data(raw):
    if not raw:
        return None
    try:
        # A API /search retorna datas como "2026-07-24T16:46:17" ou com timezone
...
def _inferir_status(dt_abertura, dt_encerramento, situacao_raw, cancelado):
    ...
    if "cancel" in situacao or "revog" in situacao:
        return "Encerrado"
    ...
    if enc is not None:
        if hoje < ab if ab else False:
            return "Aguardando Abertura"
        if hoje <= enc:
            return "Em Aberto"
        # Encerrado — pode estar em julgamento se encerrou há pouco
    ...
    if ab is not None:
        if hoje < ab:
            return "Aguardando Abertura"
        # Tem abertura mas sem encerramento — considerar aberto
        return "Em Aberto"
...
def _montar_link_pncp(numero_controle):
    """
    Converte o numeroControlePNCP para URL pública do edital.

    Formato do número: CNPJ-1-SEQUENCIAL/ANO
    Ex: "76970326000103-1-000142/2026"
    URL esperada: https://pncp.gov.br/app/editais/76970326000103/2026/142
    """
    try:
        # Remove espaços
        nc = numero_controle.strip()

        # Separa pelo "/"  → ["76970326000103-1-000142", "2026"]
        partes = nc.split("/")
        ano = partes[1].strip() if len(partes) > 1 else ""

        # Separa pelo "-" → ["76970326000103", "1", "000142"]
        segmentos = partes[0].split("-")
        cnpj = segmentos[0].strip()
        seq = str(int(segmentos[2].strip()))  # remove zeros à esquerda
    ...
    # Fallback: usa o número bruto (melhor que nada)
...
def extrair_licitacoes(dados_json):
    ...
    itens = dados_json.get("data") or dados_json.get("items") or []
    ...
    for item in itens:
        ...
        # A API /search usa data_inicio_vigencia e data_fim_vigencia
        # O endpoint de detalhe usa dataAberturaProposta / dataEncerramentoProposta
        ...
        # Monta o link
        # item_url vem como "/compras/CNPJ/ANO/SEQ" → troca para /editais/
        # numeroControlePNCP vem como "CNPJ-1-SEQANO/ANO" → precisa parsear
        item_url = item.get("item_url", "")
        if item_url:
            link = "https://pncp.gov.br/app" + item_url.replace(
                "/compras/", "/editais/"
            )
        else:
            link = primeiro(
